...
- {py:class}`~sdm.tensor.NullableTensor`: A tensor for nullable integer or boolean values.
...
- {py:attr}`~sdm.Stype.id`: identifier columns (*e.g.*, primary keys or foreign keys) represented as a {py:class}`~sdm.tensor.ColumnarTensor` with heterogeneous columns backed by {py:class}`torch.Tensor`, {py:class}`~sdm.tensor.StringTensor` or {py:class}`~sdm.tensor.NullableTensor`.
