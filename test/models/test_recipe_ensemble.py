...
def test_deterministic_model_end_to_end_includes_pre_and_postprocessing() -> (
    ...
    recipe = Recipe(
        features=[StandardScale(), _SwapFeatures()],
    ...
    feature_scale = torch.tensor([10.0, 20.0, 30.0]).std(correction=0)
    model_output = (40.0 - 20.0) / feature_scale
    target_scale = torch.tensor([1.0, 2.0, 3.0]).std(correction=0)
    canonical = model_output * target_scale + 2.0
    expected = canonical.square().reshape(1, 1)
    torch.testing.assert_close(actual, expected)


def test_cache_reuses_independent_member_recipes_and_category_mapping() -> (
    None
):
    canonical, raw = _classification_outputs()
    features = _features()
