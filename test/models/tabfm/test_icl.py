...
@pytest.mark.parametrize(
    ("field", "shape", "dtype", "match"),
    [
        ("representations", (1, 2, 4), torch.long, "representations"),
        ("representations", (1, 2, 1), torch.float, "representation width"),
        ("targets", (1, 2, 1), torch.float, "targets"),
        ("context_size", (1, 1), torch.long, "context_size"),
        ("context_size", (1,), torch.float, "context_size"),
    ],
)
def test_icl_rejects_misaligned_metadata(
    field: str,
    shape: tuple[int, ...],
    dtype: torch.dtype,
    match: str,
    ...
    inputs = {
        "representations": torch.zeros(1, 2, 4),
        "targets": torch.zeros(1, 2),
        "context_size": torch.tensor([1]),
    }
    inputs[field] = torch.zeros(shape, dtype=dtype)
    with pytest.raises(ValueError, match=match):
        _module(False)(**inputs)
...
@pytest.mark.parametrize(
    ("is_classifier", "task_keys"),
    [
        (
            True,
            "y_encoder.projection.weight y_encoder.projection.bias",
        ),
        (
            False,
            """y_encoder.layers.0.weight y_encoder.layers.0.bias
            y_encoder.layers.1.weight y_encoder.layers.1.bias""",
        ),
    ],
)
def test_icl_uses_google_state_paths(
    is_classifier: bool,
    task_keys: str,
) -> None:
    module = _module(is_classifier)
    encoder = _Encoder(
        num_blocks=1,
        channels=4,
        num_heads=2,
    ...
    encoder_keys = {f"tf_icl.{key}" for key in encoder.state_dict()}
    mlp_keys = """
    decoder.layers.0.weight decoder.layers.0.bias
    decoder.layers.1.weight decoder.layers.1.bias
    """

    expected = (
        encoder_keys | set((task_keys + mlp_keys).split()) | {"ln.weight"}
    )
    assert set(module.state_dict()) == expected
    assert module.tf_icl.rope is None


@onlyFullTest
@pytest.mark.parametrize("is_classifier", [True, False])
def test_icl_compiles_and_backpropagates(is_classifier: bool) -> None:
    module = _module(is_classifier)
    module.load_state_dict(_google_state(module))
    representations = (
        torch.arange(24, dtype=torch.float32).view(2, 3, 4).div(10)
    ).requires_grad_()
    targets = torch.tensor(
        [[0, 1, -100], [2, 1, 0]]
        if is_classifier
        ...
        dtype=torch.long if is_classifier else torch.float64,
        requires_grad=not is_classifier,
    )
    context_size = torch.tensor([1, 2])
    with torch.no_grad():
        expected = module(representations, targets, context_size)

    output = fullgraph(module)(representations, targets, context_size)
    torch.testing.assert_close(output, expected)
    output.square().sum().backward()

    assert representations.grad is not None
    assert representations.grad.isfinite().all()
    assert all(
