...
def test_cell_embedder_uses_checkpoint_state_names() -> None:
    module = _CellEmbedder(embed_dim=4, num_frequencies=2)
    ...
    assert set(module.state_dict()) == {
        ...
        "in_linear.weight",
        "in_linear.bias",
        "in_linear_cat.weight",
        "in_linear_cat.bias",
    }


def test_cell_embedder_rejects_invalid_configuration() -> None:
    with pytest.raises(ValueError, match="dimensions"):
        _CellEmbedder(embed_dim=0)
    with pytest.raises(ValueError, match="row_chunk_size"):
        _CellEmbedder(embed_dim=4, row_chunk_size=0)


@pytest.mark.parametrize(
    ("features", "categorical_mask", "active_features", "match"),
    [
        (torch.randn(2, 3), None, None, "features"),
        (torch.randn(2, 3, 4), torch.zeros(2, 4), None, "categorical_mask"),
        (torch.randn(2, 3, 4), None, torch.ones(2), "active_features"),
        (
            torch.randn(2, 3, 4),
            None,
...
def test_cell_embedder_rejects_malformed_inputs(
    features: torch.Tensor,
    categorical_mask: torch.Tensor | None,
    active_features: torch.Tensor | None,
    match: str,
) -> None:
    module = _CellEmbedder(embed_dim=4)

    with pytest.raises(ValueError, match=match):
        module(features, categorical_mask, active_features)
