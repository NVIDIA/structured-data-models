...
@pytest.mark.parametrize(
    ("is_classifier", "expected"),
    [
        (True, {"y_embedder_lookup.weight"}),
        (
            False,
            {
                "y_embedder_lookup.layers.0.weight",
                "y_embedder_lookup.layers.0.bias",
                "y_embedder_lookup.layers.1.weight",
                "y_embedder_lookup.layers.1.bias",
            },
        ),
    ],
)
def test_cell_targets_use_checkpoint_state_names(
    is_classifier: bool,
    expected: set[str],
) -> None:
    module = _CellEmbedder(
    ...
    assert {
        ...
        if key.startswith("y_embedder_lookup")
    } == expected


@pytest.mark.parametrize(
    ("targets", "context_size", "match"),
    [
        (None, None, "task-specific"),
        (torch.zeros(1, 2, 1), torch.tensor([1]), "targets"),
        (torch.zeros(1, 2), torch.tensor([[1]]), "context_size"),
        (torch.zeros(1, 2), torch.tensor([1.0]), "context_size"),
    ],
)
def test_cell_targets_reject_invalid_metadata(
    targets: torch.Tensor | None,
    context_size: torch.Tensor | None,
    match: str,
) -> None:
...
@pytest.mark.parametrize("is_classifier", [True, False])
def test_cell_targets_compile_fullgraph(is_classifier: bool) -> None:
    module = _CellEmbedder(
        embed_dim=2,
        feature_group_size=1,
        num_frequencies=1,
    ...
    features = torch.tensor([[[0.0, 0.5], [1.0, -0.5], [0.25, 0.75]]])
    targets = (
        torch.tensor([[0, 1, 2]])
        if is_classifier
        else torch.tensor([[0.5, -1.0, 0.25]])
    )
    active_features = torch.tensor([1])
