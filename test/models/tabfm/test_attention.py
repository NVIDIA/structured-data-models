...
def test_multihead_attention_block_matches_reference_and_chunking() -> None:
    torch.manual_seed(1)
    module = MultiheadAttentionBlock(
        channels=8,
        num_heads=2,
    ...
    attention = module.attn(
        query=module.pre_attn_ln(query),
        key=module.pre_attn_ln(query),
        value=module.pre_attn_ln(query),
    )
    expected = query + module.post_attn_ln(attention)
    normalized = module.pre_ff_ln(expected)
    hidden = F.silu(module.linear1_gate(normalized))
    hidden = hidden * module.linear1(normalized)
    expected = expected + module.post_ff_ln(module.linear2(hidden))

    output = module(query)
    torch.testing.assert_close(output, expected)

    chunked = copy.deepcopy(module)
    chunked.ffn_chunk_size = 3
    chunked_output = chunked(query)
    torch.testing.assert_close(chunked_output, output)

    chunked_output.square().mean().backward()
    assert query.grad is not None
    assert all(
        parameter.grad is not None for parameter in chunked.parameters()
    )


@withCUDA
@pytest.mark.parametrize("rope_theta", [None, 100_000.0])
def test_encoder(device: torch.device, rope_theta: float | None) -> None:
    module = Encoder(
        num_blocks=2,
        channels=8,
        num_heads=2,
    ...
    attn_mask = torch.ones(2, 1, 4, 4, dtype=torch.bool, device=device)
    attn_mask[:, :, :, -1] = False

    output = module(x=x, attn_mask=attn_mask)

    assert output.shape == x.shape
    assert output.dtype == x.dtype
    assert output.device == device
    if rope_theta is None:
        assert module.rope is None
    else:
        assert isinstance(module.rope, RotaryEmbedding)
        assert module.rope.layout == "interleaved"
...
@pytest.mark.parametrize(
    ("factory", "match"),
    [
        (lambda: MultiheadAttention(channels=0, num_heads=1), "channels"),
        (lambda: MultiheadAttention(channels=8, num_heads=3), "num_heads"),
        (
            lambda: MultiheadAttentionBlock(
                channels=8,
                num_heads=2,
                feedforward_channels=0,
            ),
