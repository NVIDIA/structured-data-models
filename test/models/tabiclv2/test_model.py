...
def test_tabiclv2_many_classes_forward_and_cache(
    ...
    model.fit(x_context, y_context, num_estimators=3)
    assert model.predict(x_query).allclose(out)
    # Hierarchical classification caches trees, which cannot be stacked.
    with pytest.raises(ValueError, match="cannot be batched across"):
        model.predict(x_query, estimator_batch_size=None)
