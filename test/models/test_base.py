...
def test_estimator_callbacks(estimator_batch_size: int | None) -> None:
    ...
    # Every hook runs once per member in both the forward and the fitted path.
    assert sorted(events) == sorted(
        2 * 5 * ["affine_context_preprocessing_end"]
        + 2 * 5 * ["affine_query_preprocessing_end"]
        + 2 * 5 * ["affine_model_forward_end"]
    )


def _query_sizes(calls: list[_Call]) -> list[tuple[int, ...]]:
    return [
        tuple(call.x_query.size()) if call.x_query is not None else ()
        for call in calls
    ]
...
def test_estimator_batching_runs_members_together() -> None:
    model = _RecordingModel()
    x = torch.arange(24.0).view(4, 3, 2)
    y = torch.zeros(4, 3, 1)
    ...
    assert _query_sizes(model.calls[4:]) == [(2, 3, 2), (2, 3, 2)]
    torch.testing.assert_close(out.numerical, expected.numerical)

    model.fit(x, y)
    assert len(model.calls) == 10
    out = model.predict(x, estimator_batch_size=None)
    assert _query_sizes(model.calls[10:]) == [(4, 3, 2)]
    torch.testing.assert_close(out.numerical, expected.numerical)
...
def test_estimator_batching_rejects_related_tables() -> None:
    ...
    related_tables = RelatedTables(
        tables={"related": TableTensor(numerical=torch.ones(3, 1))},
        relationships=[],
        task_links=[],
    )
    ...
    model.fit(x, y, related_tables, num_estimators=2)
    with pytest.raises(ValueError, match="Related tables require"):
        model.predict(x, related_tables, estimator_batch_size=None)
...
def test_estimator_batching_incompatible_shapes() -> None:
    ...
    with pytest.raises(RuntimeError, match="stack expects"):
        model(
            x_context=x,
            y_context=y,
            x_query=TableTensor(numerical=torch.ones(2, 2, 2)),
            estimator_batch_size=None,
        )
    ...
    queries = EnsembleTable.from_tables(
        tables=[
            TableTensor(numerical=torch.ones(1, 2)),
            TableTensor(numerical=torch.ones(2, 2)),
        ],
        member_table_ids=(0, 1),
    )
    with pytest.raises(RuntimeError, match="stack expects"):
        model.predict(queries, estimator_batch_size=None)
...
def test_estimator_batching_incompatible_categories() -> None:
    ...
    with pytest.raises(ValueError, match="share column names and classes"):
        model(
    ...
    assert model.predict(torch.ones(1, 2)).size() == (2, 1, 2)
