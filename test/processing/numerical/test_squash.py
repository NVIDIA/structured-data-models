...
def _make_input(device: torch.device) -> torch.Tensor:
    # Columns: robust spread, constant, coinciding 25th/75th percentiles,
    # robust spread with NaN and +-inf cells.
    return torch.tensor(
        [
            [-4.0, 2.0, 0.0, 1.0],
            [-1.0, 2.0, 5.0, NAN],
...
def _reference(
    ...
    lower, median, upper = np.nanpercentile(
        finite,
        [quantile_range[0], 50.0, quantile_range[1]],
        axis=0,
    )
...
def test_squash_transform_matches_reference(device: torch.device) -> None:
    ...
    torch.testing.assert_close(
        query_out.cpu(),
        torch.from_numpy(query_expected),
        equal_nan=True,
    )
