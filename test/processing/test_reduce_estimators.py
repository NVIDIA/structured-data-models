...
def test_reduce_estimators_rejects_missing_estimator_dimension() -> None:
    table = TableTensor.from_tensor(torch.ones(4, 2))

    with pytest.raises(ValueError, match="leading estimator dimension"):
        ReduceEstimators().transform(table)


def test_reduce_estimators_rejects_empty_estimator_dimension() -> None:
