...
def test_reduce_quantiles(device: torch.device) -> None:
    ...
    processor = ReduceQuantiles()

    output = processor.transform(table)

    assert not processor.requires_fit
...
def test_reduce_quantiles_rejects_non_numerical_stypes() -> None:
    table = TableTensor(
        numerical=torch.ones(2, 4, 3),
        id=ColumnarTensor((torch.arange(2 * 4).reshape(2, 4),)),
    )

    with pytest.raises(ValueError, match="numerical-only output table"):
        ReduceQuantiles().transform(table)
