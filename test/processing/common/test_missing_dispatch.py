...
def test_missing_dispatch_reuses_fitted_route_for_query() -> None:
    context = TableTensor.from_tensor(
        torch.arange(75, dtype=torch.float32).reshape(25, 3)
    )
    query = _sparse_table()
    dispatch = sp.MissingDispatch(
        dense=sp.ImputeMean(),
    ...
    assert dispatch.route == "dense"
    output = dispatch.transform(query)
    assert not output.numerical.isnan().any()
    expected = torch.where(
        query.numerical.isnan(),
        context.numerical.mean(dim=-2, keepdim=True),
        query.numerical,
    )
    torch.testing.assert_close(output.numerical, expected)


def test_missing_dispatch_first_ensemble_member_decides_route() -> None:
    sparse = _sparse_table()
    dense = sparse.replace_blocks(numerical=sparse.numerical.nan_to_num())
    dispatch = sp.MissingDispatch(min_row_frac=0.5)
    dispatch.fit_ensemble(
...
def test_missing_dispatch_counts_nan_but_not_infinity() -> None:
    table = TableTensor.from_tensor(
        torch.tensor([[1.0, float("inf")], [2.0, 3.0]])
    )
    dispatch = sp.MissingDispatch(min_row_frac=0.5).fit(table)
    assert dispatch.route == "dense"


def test_missing_dispatch_ignores_tables_without_numerical_columns() -> None:
    table = TableTensor(id=ColumnarTensor((torch.arange(4),)))
    dispatch = sp.MissingDispatch(min_row_frac=0.5)

    assert dispatch.fit_transform(table).equal(table)
    assert dispatch.route is None
    assert dispatch.transform(table).equal(table)
