...
from sdm import EnsembleTable, Recipe, StringTensor, Stype, TableTensor
...
def test_adapter_state_dict_restores_member_state_order() -> None:
    ...
    restored.fit_ensemble(
        EnsembleTable.from_table(context.table(0), num_members=1)
    )
...
def test_recipe_encodes_shared_text_before_member_fitted_transforms() -> None:
    ...
    query = EnsembleTable.from_table(query_table, num_members=num_members)
