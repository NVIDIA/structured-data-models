...
def test_tfidf_encoder_preserves_leading_dimensions() -> None:
    ...
    output = TfidfEmbed(ngram_range=(2, 2)).fit_transform(table)
...
def test_tfidf_encoder_rows_are_l2_normalized() -> None:
    ...
    output = TfidfEmbed(ngram_range=(2, 3)).fit_transform(table)
...
def test_tfidf_encoder_exact_values() -> None:
    ...
    encoder = TfidfEmbed(ngram_range=(2, 2))
...
def test_tfidf_encoder_ignores_unseen_ngrams() -> None:
    ...
    encoder = TfidfEmbed(ngram_range=(3, 3))
    encoder.fit(train)
...
def test_tfidf_encoder_max_features_keeps_most_frequent_ngrams() -> None:
    ...
    full = TfidfEmbed(ngram_range=(2, 3)).fit_transform(table)
    encoder = TfidfEmbed(ngram_range=(2, 3), max_features=3)
...
def test_tfidf_encoder_short_word_counts_once() -> None:
    ...
    output = TfidfEmbed(ngram_range=(5, 5)).fit_transform(table)
...
def test_tfidf_encoder_can_preserve_case() -> None:
    ...
    lowercased = TfidfEmbed(ngram_range=(3, 3)).fit_transform(table)
    case_sensitive = TfidfEmbed(
        ngram_range=(3, 3),
        lowercase=False,
    ).fit_transform(table)
...
def test_tfidf_encoder_cuda_matches_cpu(
    ...
    cpu_encoder = TfidfEmbed(
        ngram_range=ngram_range,
        max_features=max_features,
        lowercase=lowercase,
    )
    cuda_encoder = TfidfEmbed(
        ngram_range=ngram_range,
        max_features=max_features,
        lowercase=lowercase,
    )
...
def test_tfidf_encoder_state_dict_round_trip(tmp_path) -> None:
    ...
    encoder = TfidfEmbed(ngram_range=(2, 3))
    expected = encoder.fit_transform(table)

    path = tmp_path / "encoder.pt"
    torch.save(encoder.state_dict(), path)
    restored = TfidfEmbed(ngram_range=(2, 3))
...
def test_tfidf_encoder_load_state_dict_clears_stale_idf_buffers() -> None:
    ...
    restored = TfidfEmbed(ngram_range=(2, 2))
    restored.fit(wide)
...
def test_tfidf_encoder_refit_replaces_previous_state() -> None:
    ...
    encoder = TfidfEmbed(ngram_range=(2, 2))
    encoder.fit(wide)
    expected = TfidfEmbed(ngram_range=(2, 2)).fit_transform(narrow)
