...
def test_shuffle_categories_shift_maps_single_target() -> None:
    ...
    codes = target.categorical.index
    valid = codes >= 0
    assert torch.equal(
        output.categorical.index[valid],
        permutation[codes[valid].to(torch.long)].to(codes.dtype),
    )
    assert output.categorical.index[-1].item() == -1
    assert (
...
def test_shuffle_categories_random_permutes_each_categorical_column(
    ...
    input_codes = features.categorical.index
    output_codes = transformed.categorical.index
