...
@withCUDA
def test_align_categories_orders_negative_numeric_values(
    device: torch.device,
) -> None:
    context = TableTensor(
        ...
        categorical=CategoricalTensor(
            ...
            categories=(torch.tensor([20, -10, 10], device=device),),
        ),
    )
    query = TableTensor(
        columns={"categorical": ("value",)},
        categorical=CategoricalTensor(
            ...
            categories=(torch.tensor([10, -10, 20], device=device),),
        ),
    )
    ...
    assert torch.equal(
        ...
        torch.tensor([-10, 10, 20], device=device),
    )
    assert torch.equal(
        output.categorical.code,
...
@withCUDA
def test_align_categories_orders_boolean_values(
    device: torch.device,
) -> None:
    context = TableTensor(
        columns={"categorical": ("value",)},
        categorical=CategoricalTensor(
            ...
            categories=(torch.tensor([True, False], device=device),),
        ),
    )
    query = TableTensor(
        columns={"categorical": ("value",)},
