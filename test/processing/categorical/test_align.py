...
def test_align_categories_filters_per_ensemble_member(
    ...
    assert output.member(0).categorical.categories[0].tolist() == ["red"]
    assert output.member(0).categorical.code.squeeze(-1).tolist() == [0, 0, -1]
    assert output.member(1).categorical.categories[0].tolist() == ["blue"]
    assert output.member(1).categorical.code.squeeze(-1).tolist() == [0, 0, -1]
...
def test_align_categories_ensemble_matches_member_fits(
    ...
    for member_id, table_id in enumerate(member_table_ids):
        ...
        assert fitted_query_output.member(member_id).equal(expected_query)
