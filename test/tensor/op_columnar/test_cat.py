...
def test_cat_overload() -> None:
    first = make_columnar()
    second = make_columnar(1000)
    dense_inputs = (dense(first), dense(second))

    cat = aten.cat.default((first, second), 0)

    assert_matches_dense(cat, aten.cat.default(dense_inputs, 0))


def test_column_cat() -> None:
    first = make_columnar()
    second = make_columnar(1000)
    ...
    assert_matches_dense(
        out,
        aten.cat.default((dense(first), dense(second)), -1),
    )
