...
def test_eq(device: torch.device) -> None:
    ...
    assert torch.eq(other, left, out=out) is out
    assert not out.any()
    assert torch.ne(other, left, out=out) is out
    assert out.all()
    assert torch.eq(left, 1, out=out) is out
    assert not out.any()
    assert torch.ne(left, 1, out=out) is out
    assert out.all()
