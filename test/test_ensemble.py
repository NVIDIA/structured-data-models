...
def test_rejects_unreferenced_group_position() -> None:
    group = TableTensor.from_tensor(torch.tensor([[[1.0]], [[2.0]], [[3.0]]]))

    with pytest.raises(
        ValueError,
        match="every group position to be referenced",
    ):
        EnsembleTable(groups=(group,), locations=((0, 0), (0, 2)))
