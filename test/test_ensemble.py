...
def test_rearrange_groups_reconstructs_locations() -> None:
    ...
    assert output.num_groups == 2
    assert all(output.table(i).equal(table) for i in range(3))


def test_rearrange_groups_rejects_unavailable_sharing() -> None:
    tables = (
        TableTensor.from_tensor(torch.tensor([[1.0], [2.0]])),
        TableTensor.from_tensor(torch.tensor([[3.0], [4.0]])),
    )
    ensemble_table = EnsembleTable.from_tables(tables, (0, 1))

    with pytest.raises(ValueError, match="Cannot reconstruct"):
        ensemble_table._rearrange_groups(((0, 0), (0, 0)))

    output = ensemble_table._rearrange_groups(
        ((0, 0), (0, 0)), strict=False
    )
...
def test_rearrange_groups_rejects_unavailable_batching() -> None:
    ...
    with pytest.raises(ValueError, match="Cannot reconstruct"):
        ensemble_table._rearrange_groups(locations)

    output = ensemble_table._rearrange_groups(locations, strict=False)
    assert output._locations == ensemble_table._locations
