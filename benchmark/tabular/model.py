...
# Batching all estimators multiplies activation memory by their count.
_MAX_BATCHED_CONTEXT_CELLS = 50_000
    ...
    def _fit(
        ...
        **_: Any,
        ...
        if self._estimator_batch_size == "auto":
            # Per-estimator context subsets can fit different columns.
            if not x_context.is_cuda or self._expand_query:
                self._estimator_batch_size = 1
            elif x_context.size(-2) * x_context.size(-1) <= (
                _MAX_BATCHED_CONTEXT_CELLS
            ):
                self._estimator_batch_size = None
            else:
                self._estimator_batch_size = 1
