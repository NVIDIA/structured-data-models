"""Kumo prediction engine shared by the RelArena model and system examples."""
...
import pandas as pd
import torch
from examples.kumo.relational._relarena.history import LabelHistory
...
class KumoPredictor(RelArenaModel):
    ...
    def fit(
        ...
        *,
        ...
        self.history = None
        if config["history_lags"] not in (0, 2):
            raise ValueError("history_lags must be 0 or 2")
        if config["history_lags"]:
            self.history = LabelHistory(
                frame,
                entity_col=task.entity_col,
                time_col=task.time_col,
                target_col=task.target_col,
                time_horizon=task.timedelta,
                n_lags=2,
            )
        ...
        if self.history is not None:
            selected = pd.concat(
                [selected, self.history.transform(selected)], axis=1
            )
            stypes.update(dict.fromkeys(self.history.columns, "numerical"))
    ...
    def predict(
        ...
        if self.history is not None:
            query_frame = pd.concat(
                [query_frame, self.history.transform(query_frame)], axis=1
            )
            stypes.update(dict.fromkeys(self.history.columns, "numerical"))
