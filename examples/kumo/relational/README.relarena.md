...
The predictor and declared search space live in [rel_arena.py](rel_arena.py); the separate system script reuses them for its own selection procedure. RelArena owns hidden TEST scoring. Neither interface imports a task-specific winner map or selects using TEST results.
...
The search space in [rel_arena.py](rel_arena.py) retains 20 provisional candidates from stopped V12 after removing the ten training-label-history candidates. It covers neighborhood/hops, raw-event lags, text OFF/PCA32, standard/quantile targets, recency and entity dates. It is common across tasks and is not a completed or optimized benchmark result. The model's trial budget belongs to the caller; the system currently evaluates all 20 candidates internally.
