...
"""Zero-shot KumoTabular/TabICLv2 on Kaggle's "unimelb" competition
(Predicting Grant Applications, University of Melbourne).

Single flat table, one row per grant application: grant-level fields
(``Sponsor.Code``, ``Grant.Category.Code``, ``Contract.Value.Band``,
``Start.date``, research-field/socio-economic-objective codes and
percentages) plus up to 15 repeated blocks of investigator-level columns
(``Role.<n>``, ``With.PHD.<n>``, ``Number.of.Successful.Grant.<n>``, etc.,
for ``<n>`` in 1..15) -- most of an application's investigator-slot columns
are missing whenever it has fewer than 15 investigators, so this table is
naturally sparse; KumoTabular embeds missingness natively rather than
requiring imputation (see `itk_fall_2018.py`). ``Start.date`` is converted
to a numerical ``days_since_start`` feature relative to the latest date
across train and test (same pattern as `restaurant_revenue_prediction.py`),
since KumoTabular has no native datetime support. A trailing
``Unnamed: 251`` column (an artifact of a stray trailing comma in the
source CSV) is dropped. The binary ``Grant.Status`` target is evaluated by
AUC-ROC.
"""
...
ID_COLUMN = "Grant.Application.ID"
TARGET_COLUMN = "Grant.Status"
...
def _load(path: Path) -> pd.DataFrame:
    ...
    return frame.drop(
        columns=[c for c in frame.columns if c.startswith("Unnamed")]
    )
...
def _with_age_feature(
    ...
    frame = frame.copy()
    start_date = pd.to_datetime(frame[DATE_COLUMN], format="%d/%m/%y")
    frame[AGE_COLUMN] = (reference_date - start_date).dt.days.astype(float)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory containing unimelb_training.csv, unimelb_test.csv, "
            "and unimelb_example.csv, from `kaggle competitions download "
            "-c unimelb`."
        ),
    )
    parser.add_argument(
    ...
    train_frame = _load(args.data_dir / "unimelb_training.csv").drop(
        columns=[ID_COLUMN]
    )
    test_frame_full = _load(args.data_dir / "unimelb_test.csv")
    test_ids = test_frame_full[ID_COLUMN]
    test_frame = test_frame_full.drop(
        columns=[c for c in (ID_COLUMN, TARGET_COLUMN) if c in test_frame_full]
    )
