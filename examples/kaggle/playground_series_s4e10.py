...
"""Zero-shot KumoTabular/TabICLv2 on Kaggle's Playground Series S4E10 (loan
approval prediction).

Single flat table: numerical applicant/loan fields (``person_age``,
``person_income``, ``loan_amnt``, ``loan_int_rate``,
``loan_percent_income``, ``cb_person_cred_hist_length``) and categorical
fields (``person_home_ownership``, ``loan_intent``, ``loan_grade``,
``cb_person_default_on_file``), with no missing values in either split.
The binary ``loan_status`` target is evaluated by AUC-ROC -- the simplest
dataset in this directory, structurally identical to
`santander_customer_transaction_prediction.py` but with named rather than
anonymized features.
"""
