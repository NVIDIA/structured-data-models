...
"""Zero-shot KumoTabular/TabICLv2 on Kaggle's Bioresponse competition.

Single flat table: a binary ``Activity`` target and ~1776 numerical
molecular descriptor columns (``D1``..``D1776``). Notably, ``test.csv`` has
no id column -- submissions (``svm_benchmark.csv`` is the competition's
baseline/reference file) are keyed by 1-based row position, same convention
as `examples/kaggle/itk_fall_2018.py`. Evaluated by log loss.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory containing train.csv and test.csv, from `kaggle "
            "competitions download -c bioresponse`."
        ),
    )
    parser.add_argument(
        "--model",
    ...
    for split_train_index, split_val_index in splitter.split(
        ...
        losses.append(
            log_loss(target.cpu().numpy(), scores.float().cpu().numpy())
        )
    ...
    submission = pd.DataFrame(
        {
            "MoleculeId": np.arange(1, len(positive_scores) + 1),
            "PredictedProbability": positive_scores,
        }
    )
