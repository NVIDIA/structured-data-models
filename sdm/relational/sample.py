...
class RelationalSample(DeviceMixin):
    ...
    def __post_init__(self) -> None:
        if self.num_hops < 0:
            raise ValueError("Expected 'num_hops' to be non-negative")
        if self.num_hops != len(self.num_neighbors):
            raise ValueError(
                "Expected 'num_hops' to equal the configured fanout depth"
            )
        if any(num_neighbors < -1 for num_neighbors in self.num_neighbors):
            raise ValueError("Expected neighbor fanouts to be at least -1")
        ...
        for table_name, batch in self.node_batch.items():
            ...
            _validate_integer_vector(batch, f"batch for table {table_name!r}")
            ...
            if hop.numel() > 0 and (
                bool((hop < 0).any()) or bool((hop > self.num_hops).any())
            ):
                raise ValueError(
                    f"Expected node hops for table {table_name!r} to be "
                    f"between 0 and {self.num_hops}"
                )
        ...
        for index, (edge_index, edge_hop) in enumerate(
            ...
            if edge_hop.numel() > 0 and (
                bool((edge_hop < 1).any())
                or bool((edge_hop > self.num_hops).any())
            ):
                raise ValueError(
                    f"Expected edge hops at index {index} to be between 1 "
                    f"and {self.num_hops}"
                )
