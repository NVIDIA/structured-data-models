        ...
        **kwargs: Any,
        ...
        Args:
            ...
            estimator_batch_size: Maximum number of estimators run through the
                ...
                memory grows with the batch size. Estimators fitted together
                are predicted together. Model-side randomness drawn per call
                (*e.g.*, ECOC codebooks for more than 10 classes) is shared
                within a batch, so batched and sequential predictions differ
                numerically there.
            ...
            with inference_mode("no_grad"):
                self._forward(
                    ...
                    categorical_mask=cast(
                        Tensor, batch_cache["categorical_mask"]
                    ),
                    **kwargs,
                )
    ...
    def _forward(
        ...
        **kwargs: Any,
        ...
        Returns:
            ...
            hold one column per class in the order of
            ``y_context.categorical.categories[0]`` (``cache["classes"]`` on
            replay), named by class value; :class:`ICLModel` relabels them
            per estimator when stacked.
        """
