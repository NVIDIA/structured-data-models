...
class ICLModel(torch.nn.Module, abc.ABC):
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            num_estimators: The number of estimators ``E`` for ensembling.
                ...
                per estimator).
            estimator_batch_size: The maximum number of estimators run through
                the model in one call. ``1`` runs estimators one by one, and
                ...
                batch size. Model-side randomness drawn per call (*e.g.*, the
                ECOC codebooks of :class:`~sdm.models.KumoTabular` for more
                than ten classes) is shared within a batch. Estimators fitted
                together are predicted together.
        ...
        *,
        ...
        try:
            ...
            for i in range(num_batches):
                ...
                schemas = cast(MemberSchemas, cache["schemas"])
                related_schema = cast(
                ...
                members = [
                    self._prepare_query(
                        query, x_schema, related_schema, callbacks
                    )
                    for query, x_schema in zip(
                        queries[start : start + len(schemas.members)],
                        schemas.members,
                        strict=True,
                    )
                ]
    ...
    def _forward(
        ...
        **kwargs: Any,
        ...
        r"""Run the model on the preprocessed tables of one estimator batch.

        Tables have shape ``[E, ..., R, D]`` when ``E > 1`` estimators run
        together and ``[..., R, D]`` otherwise. Stacked tables carry the
        column names and categories of the first estimator only;
        ``schemas.members[e]`` holds the preprocessed feature schema of
        estimator ``e``, ``schemas.inputs[e]`` its feature schema before
        ``recipe.features``, and :meth:`MemberSchemas.stype_mask` derives
        per-column provenance from them. Values written to ``cache`` are
        replayed with queries of the same estimator batch. Classification
        outputs hold one column per class in the order of
    ...
    def _requires_grad(self, callbacks: Sequence[Callback]) -> bool:
        return self.training or any(c.requires_grad for c in callbacks)
    ...
    def _prepare_query(
        ...
        related_schema: RelatedTablesSchema | None,
        callbacks: Sequence[Callback],
    ) -> MemberQuery:
        for callback in callbacks:
            query = MemberQuery(
        ...
        self._validate_query(
            x_context=x_schema,
            x_query=query.x,
            related_context_tables=related_schema,
    ...
    def _forward_batch(
        ...
        **kwargs: Any,
        ...
        # Stack inside the grad region so callback-captured leaves stay
        ...
        with inference_mode("grad" if requires_grad else "inference"):
            ...
            outs = _unstack(stacked, class_values, len(queries))
    ...
    def _forward_members(
        ...
        **kwargs: Any,
        ...
        r"""Run preprocessed members that live on the model device.
        ...
        for start in range(0, len(contexts), estimator_batch_size):
            ...
            query_members = [
                self._prepare_query(
                    ...
                    related_schema=None
                    if member.related_tables is None
                    else member.related_tables.schema,
                    callbacks=callbacks,
                )
...
def _check_compatible(tables: Sequence[TableTensor]) -> None:
    ref = tables[0]
    names = {stype: frozenset(cols) for stype, cols in ref.columns.items()}
    counts = tuple(c.numel() for c in ref.categorical.categories)
    for table in tables[1:]:
        if {
            stype: frozenset(cols) for stype, cols in table.columns.items()
        } != names or (
...
def _stack_context(members: Sequence[MemberContext]) -> MemberContext:
    ...
    return MemberContext(
        ...
        related_tables=None,
        input_schema=members[0].input_schema,
    )
...
def _schemas(members: Sequence[MemberContext]) -> MemberSchemas:
    return MemberSchemas(
        inputs=tuple(member.input_schema for member in members),
        members=tuple(member.x.schema for member in members),
    )
...
def _member_columns(
    ...
    if class_values is None or len(labels) != len(class_values[0]):
        return None
    # The model labels classes in the first member's order; column `j` of
    # member `e` holds its class `class_values[e][j]`.
    index = {value: i for i, value in enumerate(class_values[0])}
    return tuple(
        tuple(labels[index[value]] for value in values)
...
def _unstack(
    ...
    columns = _member_columns(out.columns[Stype.numerical], class_values)
    if columns is None:
        return outs
    return [
        TableTensor(columns={Stype.numerical: names}, numerical=out.numerical)
        for out, names in zip(outs, columns, strict=True)
    ]
