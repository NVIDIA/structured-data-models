...
class RowEmbedding(torch.nn.Module):
    ...
    def forward(
        ...
        *B, R, C = x.size()
        ...
        # Only the lower bound is applied here: a non-positive count would
        # mask out every in-context row and silently return degenerate
        # numbers, and rejecting it would require reading the values off the
        # device on every call. The upper bound is applied in `SDPA` against
        # the actual key length (fresh or cached), where the count is used.
        seqused_col = (
        ...
        # Reuse the clamped count so under-range values saturate like the
        # feature-grouping gather above instead of masking every real key.
        attn_mask = None
        if seqused_cols is not None:
            key_index = torch.arange(K + C, device=x.device)
            attn_mask = (key_index < K + num_cols).view(1, K + C)
