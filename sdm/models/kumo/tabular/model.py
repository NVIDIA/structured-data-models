    ...
    def _forward_members(
        ...
        **kwargs: Any,
        ...
        if len(context_size) != 2 or any(
            ...
            for x_context, x_query in zip(x_contexts, x_queries, strict=True)
        ):
            return None
        ...
        if is_categorical[0]:
            ...
            if max(len(c) for c in classes) > 10:
                raise ValueError(
                    f"{self.__class__.__name__!r} only supports up to 10 "
                    f"classes (got {max(len(c) for c in classes)})"
                )
            y = torch.stack(
        ...
        categorical_mask = torch.tensor(
            ...
            dtype=torch.bool,
        ).to(device, non_blocking=True)  # [E, D]

        out = torch.cat(
            [
                self.models[task](
                    x[start : start + group_size],
                    y[start : start + group_size],
                    categorical_mask[start : start + group_size],
                )
